...
export function EvidenceSources({ citations, googleQueries, hackerNewsHits, warnings = [] }: EvidenceSourcesProps) {
    ...
    const activeWarnings = warnings.filter(w => !dismissedWarnings.has(w.message));
            ...
            {activeWarnings.length > 0 && (
                <div className="space-y-3">
                    {activeWarnings.map((warning, idx) => (
                        <div key={idx} className={`p-4 rounded-xl border flex items-start gap-3 ${warning.type === "missing_config"
                            ...
                            <div className="flex-1">
                                <h3 className="font-medium text-sm">{warning.message}</h3>
                                {warning.details && (
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    newDismissed.add(warning.message);
